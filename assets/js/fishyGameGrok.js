...
const fishTypes = [
    { name: "🐟 Common Bass", rarity: 0.6, color: "#4fa3a5", scale: 1.1 },
    { name: "🐠 Tropical Guppy", rarity: 0.2, color: "#ff8c00", scale: 0.85 },
    { name: "🐡 Blowfish", rarity: 0.1, color: "#ead864", scale: 1.3 },
    { name: "🦈 Rare Shark", rarity: 0.04, color: "#4b5d67", scale: 2.4 },
    { name: "✨ Golden Koi", rarity: 0.01, color: "#ffd700", scale: 1.6 },
    { name: "👞 Old Boot", rarity: 0.05, color: "#5d4037", scale: 1.0 }
];
...
function spawnFishingEvent(user, userColor) {
    ...
    setTimeout(() => {
        if (fisher.state === 'waiting') triggerBite(fisher);
    }, waitTime);
}
...
function drawFisher(fisher) {
    ...
    ctx.fillRect(fisher.x - 22, fisher.y - 88, 18, 55);           // torso
    ctx.beginPath();
    ctx.arc(fisher.x - 13, fisher.y - 98, 11, 0, Math.PI * 2);    // head
    ctx.fillStyle = fisher.color;
    ctx.fill();
    ...
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(fisher.x - 8, fisher.y - 75);
    ctx.lineTo(fisher.x + 18, fisher.y - 105);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.moveTo(fisher.x + 18, fisher.y - 105);
    ctx.quadraticCurveTo(fisher.x + 35, fisher.y - 125, fisher.x + 22, fisher.y - 148);
    ctx.stroke();
    ctx.shadowBlur = 0;
    ...
    ctx.beginPath();
    ctx.arc(fisher.x + 24, fisher.y - 118, 9, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.lineWidth = 1.8;
    ctx.beginPath();
    ctx.moveTo(fisher.x + 22, fisher.y - 148); // rod tip
    ctx.quadraticCurveTo(fisher.x + 40, fisher.y - 110, fisher.bobberX, fisher.bobberY);
    ctx.stroke();
}
...
function drawCaughtFish(fisher) {
    ...
    ctx.font = `${32 * fisher.fish.scale}px Arial`;
    ...
    ctx.textAlign = "center";
    ctx.fillText(fisher.fish.name, fishX, fishY + 38);
    ctx.shadowBlur = 0;
}
...
function animate() {
    ...
    activeFishers.forEach(f => {
        // Casting animation (smooth + easing)
        if (f.state === 'casting') {
        ...
        if (f.state === 'reeling') {
            ...
            const rodTipX = f.x + 22;
            const rodTipY = f.y - 148;
