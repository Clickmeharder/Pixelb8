...
const original=page.widgets.find(w=>w.id===activeWidgetId);
if(!original)return;
...
break;
case"button":
el.innerHTML="<a href='#' class='pb8-btn' contenteditable>"+w.content+"</a>";
break;
}
...
const frame=document.getElementById("frame");
if(!codeView){
frame.innerText=JSON.stringify(PB8.pages[PB8.currentPage],null,2);
}else{
Renderer.render();
}
codeView=!codeView;
...
const Export = {
html(){
alert("Export system ready for expansion.");
}
};
