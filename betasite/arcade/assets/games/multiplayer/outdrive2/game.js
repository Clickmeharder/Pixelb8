...
window.addEventListener('keydown', event => {
    ...
    if (gameState === 'MAIN_MENU') {
        if (key === '5') {
            if (typeof openOutDriveMultiplayer === 'function') openOutDriveMultiplayer();
            return;
        }
        if (key === '3' || lowerKey === 'g') {
            garageVisited = true;
...
function drawRoadAndScenery() {
    ...
    if (typeof getMultiplayerRenderCars === 'function') {
        getMultiplayerRenderCars().forEach(car => {
            if (car.z > pos + CONFIG.nearDistance && car.z < pos + CONFIG.viewDistance) {
                objects.push({
                    type: car.vehicleType || 'RACER',
                    z: car.z,
                    x: car.x,
                    params: car
                });
...
function drawMainMenu() {
    ...
    ctx.font = 'bold 14px "Courier New", monospace'; ctx.fillStyle = '#62ff8c'; ctx.fillText('5  ONLINE MULTIPLAYER', WIDTH / 2, 244);
    ctx.font = '8px "Courier New", monospace'; ctx.fillStyle = '#baffd1'; ctx.fillText('Host, join or spectate live races', WIDTH / 2, 257);

    ctx.font = '7px "Courier New", monospace'; ctx.fillStyle = handlingAssist ? '#62ff8c' : '#ff7777';
    ctx.fillText(`ASSIST ${handlingAssist ? 'ON' : 'OFF'} • AUTO SHIFT ${autoShift ? 'ON' : 'OFF'} • CRT ${crtEnabled ? 'ON' : 'OFF'}`, WIDTH / 2, 273);
    ctx.fillStyle = '#66727e'; ctx.fillText('Type JAEDEV for development unlock', WIDTH / 2, 288);
    ctx.textAlign = 'left';
}
...
function drawRace() {
    ...
    drawTireEffects();
    if (typeof isMultiplayerSpectator !== 'function' || !isMultiplayerSpectator()) {
