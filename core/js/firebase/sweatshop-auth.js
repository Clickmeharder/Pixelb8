...
*/
  ...
  import { getFirestore, collection, getDocs, doc, getDoc, addDoc, setDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/10.10.0/firebase-firestore.js";
      ...
      if (!itemsSnapshot.empty) {
        ...
        itemsSnapshot.forEach((itemDoc, index) => {
          ...
          if (userRole === 'admin' || itemData.ownerId === userUid) {
            docHTML += `
              <tr style="background: ${index % 2 === 0 ? 'linear-gradient(to right, #071223, #0f2c48)' : 'linear-gradient(to left, #071223, #0f2c48)'};">
                <td>${itemDoc.id}</td>
