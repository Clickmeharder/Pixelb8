...
// Get the input fields and display elements
const totalBudgetInput = document.getElementById('ValueInput-eventsbudgetTotal');
const feesPercentageInput = document.getElementById('allocationpercent-eventfees');
const prizesPercentageInput = document.getElementById('allocationpercent-eventprizes');
const feesDisplay = document.getElementById('Value-eventsbudgetFees');
const prizesDisplay = document.getElementById('Value-eventsbudgetPrizes');


// Function to update the displayed values based on the percentages
// Function to update the displayed values based on the percentages
function updateEventBudgetValues() {
    // Get the total budget value
    const totalBudget = parseFloat(totalBudgetInput.value);

    // Get the percentages entered by the user
    const feesPercentage = parseFloat(feesPercentageInput.value) / 100; // No need to replace "%" here
    const prizesPercentage = parseFloat(prizesPercentageInput.value) / 100; // No need to replace "%" here

    // Calculate the fees and prizes values
    const feesValue = totalBudget * feesPercentage;
    const prizesValue = totalBudget * prizesPercentage;

    // Update the displayed values
    feesDisplay.textContent = feesValue.toFixed(2); // Set to 2 decimal places
    prizesDisplay.textContent = prizesValue.toFixed(2); // Set to 2 decimal places


    // Store the total budget value in local storage
    localStorage.setItem('totalBudget', totalBudget);
}
function addPercentSymbol(event) {
    const input = event.target;
    let value = input.value.trim(); // Remove any leading or trailing whitespace
    if (!value.includes('%')) { // Check if '%' is not already present
        value += '%'; // Append '%' if it's not already there
        input.value = value; // Update the input value
        console.log("addpercent if statement ran");
    }
}

// Load the total budget value from local storage if available
const storedTotalBudget = localStorage.getItem('totalBudget');
if (storedTotalBudget) {
    totalBudgetInput.value = storedTotalBudget;
}


// Add event listener to update local storage when the total budget input changes
totalBudgetInput.addEventListener('input', () => {
    localStorage.setItem('totalBudget', totalBudgetInput.value);
});
// Add event listeners to the percentage input fields
feesPercentageInput.addEventListener('input', () => {
	addPercentSymbol(event);
    updateEventBudgetValues(); // Update the displayed values
	
});

prizesPercentageInput.addEventListener('input', () => {
	addPercentSymbol(event);
    updateEventBudgetValues(); // Update the displayed values
});
