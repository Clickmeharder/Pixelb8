...
/* function updateTimers() {
    ...
    // 1. Update individual mission logic
    missions.forEach(m => {
        // If a mission has a timer set AND that time has passed
        if (m.readyAt > 0 && m.readyAt <= now) {
            m.readyAt = 0; // Mark as ready
            missionFinished = true; // Flag to trigger UI update and sound
            
            // 🔊 TRIGGER THE SOUND
            // This calls your existing global sound system
            if (typeof window.playSound === 'function') {
                window.playSound('mission-done');
            }
			// 🗣️ TTS: Say the mission is ready
            if (typeof window.speakText === 'function') {
                // Short delay so it doesn't overlap with the sound effect
                setTimeout(() => {
    ...
    // 2. Refresh the UI if any timer finished
    if (missionFinished) {
        saveAndRender(); // Saves the 'ready' state and updates the HTML
        return; 
    }

    // 3. Update the text displays for missions still counting down
    ...
    // 4. Update planet header "Next:" timers
