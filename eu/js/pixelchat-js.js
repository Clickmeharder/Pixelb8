...
import { getFirestore, collection, getDocs, doc, getDoc, addDoc, orderBy, query, startAfter, limit, onSnapshot } from "https://www.gstatic.com/firebasejs/10.10.0/firebase-firestore.js";
...
let observer = new IntersectionObserver((entries) => {
  ...
  if (entry.intersectionRatio && entry.isIntersecting) {
    ...
    let queryRef = query(chatsRef, orderBy("time", "desc"), limit(10));
    
    if (startAt) {
      queryRef = query(queryRef, startAfter(startAt));
    }
...
getDocs(query(chatsRef, orderBy("time", "desc"), limit(10)))
  .then((snap) => {
    const messages = snap.docs.map((doc) => ({...doc.data(), id: doc.id}));
...
onSnapshot(query(chatsRef, orderBy("time"), startAfter(Date.now())), (snapshot) => {
...
send.onclick = function () {
  if (user.value && text.value) {
    ...
    sendMessage(user.value, text.value);
  } else {
    alert(
