		...
		// --- OBS CONSOLE BRIDGE ---
		// This captures all console logs and redirects them to your UI
		...
		function displayConsoleMessage(user, message) {
			...
			if (!consoleContainer) return; // Safety check for early logs
				...
				case "toggle":
					...
					if (target === "consolemessages") {
						...
						botSay(`Prefix Mode: ${useCmdPrefix ? "REQUIRED (!" + CMD_PREFIX + ")" : "Disabled"}`);
									...
									botSay(`Floating Emotes are now: ${useBotPrefix ? "Enabled" : "Disabled"}`);
					}
					saveSettings();
					break;
        ...
        function startTwitch(channel, token) {
			...
			ComfyJS.onReward = (user, reward, cost, message, extra) => {
				...
				if (reward === "Fart") {
					alertbox.innerText = `${user.toUpperCase()} farted!`;
				}
				botSay(`@${user}! You just stink.`);
